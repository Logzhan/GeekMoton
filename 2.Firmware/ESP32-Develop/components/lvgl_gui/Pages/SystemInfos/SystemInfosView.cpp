...
{
    Item_Delete(&ui.sport);
   // Item_Delete(&ui.gps);
    Style_Reset();
    printf("SystemInfosView::Delete() finsih\n");
}
...
void SystemInfosView::Item_CreateHelper(
    item_t* item,
    lv_obj_t* par,
    const char* name,
    const char* img_src,
...
{
    return Item_CreateHelper(item,par,name,img_src,infos);

    //lv_obj_t* cont = lv_obj_create(par);
    //lv_obj_remove_style_all(cont);
    //
    //lv_obj_set_width(cont, 220);
    //lv_obj_set_height(cont, 135);
    //lv_obj_clear_flag(cont, LV_OBJ_FLAG_SCROLLABLE);
    ...
    //lv_img_set_src(img, ResourcePool::GetImage(img_src));
    //lv_obj_align_to(img, NULL, LV_ALIGN_CENTER, -80, -20);
    //item->icon = img;
    ...
    //lv_obj_align_to(label, img, LV_ALIGN_CENTER, 0, 25);
    //item->iconName = label;
...
void SystemInfosView::Item_Delete(item_t* item){
    // lv_obj_del(item->labelInfo);
    // lv_obj_del(item->labelData);
    // lv_obj_del(item->iconImage);
    // lv_obj_del(item->icon);
    lv_obj_del(item->icon);
}
